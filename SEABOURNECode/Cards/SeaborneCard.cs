...
) : CustomCardModel(cost, type, rarity, target)
{
    ...
    protected Task Deal(CardPlay play, decimal damage) => CommonActions.CardDamage(this, play, SeaborneCardRuntime.ModifyOutgoingDamage(play, damage));
    protected Task DealAll(CardPlay play, decimal damage) => CommonActions.CardDamageToAllEnemies(this, play, SeaborneCardRuntime.ModifyOutgoingDamage(play, damage));
    protected Task GainBlock(CardPlay play, decimal block) => CommonActions.CardBlock(this, play, SeaborneCardRuntime.ModifyOutgoingBlock(play, block));
    protected Task Gain(CardPlay play, object power, int amount) => CommonActions.GainPower(this, play, power, amount);
    protected Task Apply(CardPlay play, object power, int amount) => CommonActions.ApplyPower(this, play, power, SeaborneCardRuntime.ModifyBuffDebuffStacks(play, amount));
    protected Task ApplyAll(CardPlay play, object power, int amount) => CommonActions.ApplyPowerToAllEnemies(this, play, power, SeaborneCardRuntime.ModifyBuffDebuffStacks(play, amount));

    protected Task Cast(CardPlay play, int amount) => SeaborneCardRuntime.Cast(play, amount);
    protected Task Reel(CardPlay play) => SeaborneCardRuntime.Reel(play, null);
    ...
    protected Task AddCannonballs(CardPlay play, int count) => SeaborneCardRuntime.AddCannonballs(play, count, false);
    protected Task AddSpinyCannonballs(CardPlay play, int count) => SeaborneCardRuntime.AddCannonballs(play, count, true);
}
