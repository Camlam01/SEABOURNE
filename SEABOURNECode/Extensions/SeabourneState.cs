...
public static class SeabourneState
{
    ...
    {
        var data = Card(card);
        return data.WetStacks > 0
               || data.ImbuedStacks > 0
               || data.Unimbued
               || data.TemporaryCostDelta != 0
               || data.ExhaustFromAmber;
    }
    ...
    public static decimal ApplyDamageMods(SeabourneCard card, CardPlay play, decimal value)
    {
        ...
        if (Gems(player).Charged.Contains(SeabourneGemType.Ruby) && value > 0m)
        {
            result += value * 0.2m * multiplier;
            Gems(player).Charged.Remove(SeabourneGemType.Ruby);
        }

        if (creature is not null && HasPower<ShardyShrapnelPower>(creature, out var shrapnel))
    ...
    public static decimal ApplyBlockMods(SeabourneCard card, CardPlay play, decimal value)
    {
        ...
        if (Gems(player).Charged.Contains(SeabourneGemType.Sapphire) && value > 0m)
        {
            result += value * 0.2m * multiplier;
            Gems(player).Charged.Remove(SeabourneGemType.Sapphire);
        }
    ...
    public static int ApplyStackMods(SeabourneCard card, CardPlay play, int amount)
    {
        ...
        if (Gems(player).Charged.Contains(SeabourneGemType.Emerald))
        {
            result += multiplier;
            Gems(player).Charged.Remove(SeabourneGemType.Emerald);
        }

        if (creature is not null && HasPower<JinxPower>(creature, out var jinx))
        {
    ...
    public static int ApplyCastMods(SeabourneCard card, CardPlay play, int amount)
    {
        ...
        if (Gems(player).Charged.Contains(SeabourneGemType.Opal))
        {
            result += multiplier;
            Gems(player).Charged.Remove(SeabourneGemType.Opal);
        }

        if (creature is not null && HasPower<MasterbaitPower>(creature, out var masterbait))
    ...
    public static void ApplyCostAndWetMods(SeabourneCard card, CardPlay play)
    {
        ...
        if (Gems(player).Charged.Contains(SeabourneGemType.Amber))
        {
            data.TemporaryCostDelta -= Math.Max(1, multiplier);
            data.ExhaustFromAmber = true;
            Gems(player).Charged.Remove(SeabourneGemType.Amber);
        }

        if (Gems(player).Charged.Contains(SeabourneGemType.Diamond))
        {
            data.WetStacks += Math.Max(1, multiplier);
            Gems(player).Charged.Remove(SeabourneGemType.Diamond);
        }
    }

    public static bool HasPower<TPower>(object? owner, out TPower? power)
        where TPower : class
    {
    ...
    public static void GainEnergy(object owner, int amount)
    {
        ...
        var gainEnergyMethod = playerCmdType?
            .GetMethods()
            .FirstOrDefault(m => m.Name == "GainEnergy" && m.GetParameters().Length >= 2);
    ...
    public static void ResetTurn(object owner)
    {
        ...
        var creature = SeabourneReflection.GetProp<Creature>(owner, "Creature", "OwnerCreature", "PlayerCreature");
        turn.EnchantedRodRemaining = creature is not null && HasPower<EnchantedRodPower>(creature, out var rod)
            ? rod.Amount
            : 0;
