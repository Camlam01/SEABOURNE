...
public sealed class TheSeaborne : CustomCharacterModel
{
    ...
    public override IEnumerable<string> StartingDeck =>
    [
        ModelDb.Card<StrikeCard>().Id,
        ModelDb.Card<StrikeCard>().Id,
        ModelDb.Card<StrikeCard>().Id,
        ModelDb.Card<StrikeCard>().Id,
        ModelDb.Card<DefendCard>().Id,
        ModelDb.Card<DefendCard>().Id,
        ModelDb.Card<DefendCard>().Id,
        ModelDb.Card<DefendCard>().Id,
        ModelDb.Card<FishCard>().Id,
        ModelDb.Card<FireCannonCard>().Id,
    ];

    public override IReadOnlyList<string> StartingRelics =>
    [
        ModelDb.Relic<SeaborneStarterRelic>().Id,
    ];
